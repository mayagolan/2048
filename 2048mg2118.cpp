...
bool proc_num(std::vector<int>& v, int bi, int ei){
    ...
    /// you should not need any temporary vectors to perform this function
    for (int i = bi; i < ei; i++){
        if (v[i] != 0) {
...
void randomGen(std::vector<int>&v){
    ...
    if(zeroPresent){
        ...
        /// you need to seed your random generator with srand
        while(v[randomIndex] != 0 ){
            randomIndex = rand() % v.size();
        }
        /// this loop can get stuck for a while if e.g. there is only one zero
        /// it is better to first look for all zeros and then choose one of them
        v[randomIndex] = 2;
    }
}
